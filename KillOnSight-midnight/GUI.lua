...
local pPlayers = CreateFrame("Frame", nil, frame)
  ...
  remBtn:SetPoint("TOPRIGHT", pPlayers, "TOPRIGHT", -10, -10)
  ...
  gRemBtn:SetPoint("TOPRIGHT", pGuilds, "TOPRIGHT", -10, -10)
  ...
  if not StaticPopupDialogs["KOS_RESET_STATS"] then
    StaticPopupDialogs["KOS_RESET_STATS"] = {
      ...
      OnAccept = function()
        ...
        if DB and DB.ClearLastAttackers then DB:ClearLastAttackers() end
        local Stats = _G.KillOnSight_MidnightStats
        if Stats and Stats.OnResetStats then
          pcall(Stats.OnResetStats, Stats)
        end
...
local cNearbyPing = MakeCheck(opt, L.UI_NEARBY_PING)
cNearbyPing:SetPoint("TOPLEFT", cNearbySound, "BOTTOMLEFT", 0, -6)
cNearbyPing:SetChecked(prof.nearbyPingOnClick ~= false)
cNearbyPing:SetScript("OnClick", function(self)
  prof.nearbyPingOnClick = self:GetChecked()
end)

-- Description text for ping toggle
local tPingHelp = opt:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
tPingHelp:SetPoint("TOPLEFT", cNearbyPing, "BOTTOMLEFT", 26, -2)
tPingHelp:SetWidth(340)
tPingHelp:SetJustifyH("LEFT")
tPingHelp:SetText(L.UI_NEARBY_PING_HELP)
tPingHelp:SetTextColor(0.6, 0.6, 0.6)

local cNearbyLock = MakeCheck(opt, L.UI_NEARBY_LOCK)
cNearbyLock:SetPoint("TOPLEFT", tPingHelp, "BOTTOMLEFT", -26, -10)
